package uk.ac.cam.cl.gfxintro.dm894.RubiksCubeSolver.Rendering;
...
public class OpenGLApplication {
	...
	public void initialize() throws Exception {
		...
		window = glfwCreateWindow(WIDTH, HEIGHT, "Rubiks Cube Solver", NULL, NULL);
		if (window == NULL)
		...
		key_cb = new GLFWKeyCallback() {
			public void invoke(long window, int key, int scancode, int action, int mods) {
				if (key == GLFW_KEY_W && action == GLFW_PRESS) {
					...
					rubiksCubeManager.solveLayer();
				} else if (key == GLFW_KEY_R && action == GLFW_PRESS) {
					if(prime){
						rubiksCubeManager.makeMove("RP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("R");
					}
				} else if (key == GLFW_KEY_L && action == GLFW_PRESS) {
					if(prime){
						rubiksCubeManager.makeMove("LP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("L");
					}
				} else if (key == GLFW_KEY_U && action == GLFW_PRESS) {
					if(prime){
						rubiksCubeManager.makeMove("UP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("U");
					}
				} else if (key == GLFW_KEY_X && action == GLFW_PRESS) {
					if(prime){
						rubiksCubeManager.makeMove("xP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("x");
					}
				} else if (key == GLFW_KEY_Y && action == GLFW_PRESS) {
					if(prime){
						rubiksCubeManager.makeMove("yP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("y");
					}
					...
					if(prime){
						rubiksCubeManager.makeMove("FP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("F");
					}
				} else if (key == GLFW_KEY_M && action == GLFW_PRESS) {
					if(prime){
						rubiksCubeManager.makeMove("MP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("M");
					}
				} else if (key == GLFW_KEY_B && action == GLFW_PRESS) {
					if(prime){
						rubiksCubeManager.makeMove("BP");
						prime = false;
					} else {
						rubiksCubeManager.makeMove("B");
					}
	...
	public void render() {
		...
		if(!pause) {
			...
			rubiksCubeManager.renderCube(camera);
